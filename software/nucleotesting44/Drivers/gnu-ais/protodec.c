...
{
	//puts("hi this is protodec.c speaking");
    ...
    // clear the file on init/between passes
    //fclose(fopen("AIS_log.txt", "w"));
    //d->saved_ais_messages = hmalloc()
	//d->array = malloc(sizeof(*d->array) * 10);
	//	d->array = malloc(sizeof(int[10][50]));
...
uint16_t protodec_sdlc_crc(const uint8_t *data, uint32_t len)
{
...
int protodec_calculate_crc(int32_t length_bits, struct demod_state_t *d)
{
	...
	/* what is this?*/
	buf = (uint8_t *) malloc(sizeof(*buf) * buflen);
 	for (j = 0; j < buflen; j++) {
		tmp = 0;
 		for (i = 0; i < 8; i++)
	...
	uint16_t crc = protodec_sdlc_crc(buf, buflen);
	//DBG(printf("CRC: %04x\n",crc));
...
uint32_t protodec_henten(int32_t from, int32_t size, uint8_t *frame)
{
	int32_t i = 0;
	uint32_t tmp = 0;
...
void protodec_generate_nmea(struct demod_state_t *d, int32_t bufferlen, int32_t fillbits, time_t received_t)
{
    int32_t senlen;
    int32_t pos;
    int32_t k, offset;
    ...
    uint8_t sentences, sentencenum, nmeachk, letter;
    // below line MARKED OUT BY JANNE due to error
  //  received_t=received_t; // not used here, avoid compiling warnings
	...
	do {
        ...
        while (k < senlen + offset && bufferlen > pos) {
            letter = (uint8_t)protodec_henten(pos, 6, d->rbuffer);
...
void protodec_getdata(int32_t bufferlen, struct demod_state_t *d)
{
...
//	unsigned long mmsi = protodec_henten(8, 30, d->rbuffer);
	...
	// -------------- temp print and save to log
	/* crashes when running on stm32 -> commented out.
	printf("\n Decoded message receiver %c at protodec.c in NMEA format: %s ", d->chanid, d->nmea);

    FILE *fp_out_I;
    fp_out_I = fopen("AIS_log.txt", "a");
    fseek(fp_out_I, 0, SEEK_END);
    fprintf(fp_out_I, "%s", d->nmea);
    fclose(fp_out_I);
	*/
    // -----------------------------
	...
	if (type < 1 || type > MAX_AIS_PACKET_TYPE)
		return; // unsupported packet type
									...
									// samplenum not in the original gnuais, only used once below
void protodec_decode(char *in, int32_t count, struct demod_state_t *d, uint32_t samplenum)
{
//	printf("char *in value from protodec_decode is: %d \n", *in);
	...
	if(d->debug)
		//printf("nskurr = %d,  npreamble = %d,  nstartsign = %d,  ndata = %d,  nstopsign = %d, aenner = %d, areamble = %d, bitstuff = %d  sample %u, receiver: %c ", d->nskurr, d->npreamble, d->nstartsign, d->ndata, d->nstopsign, d->antallenner,d->antallpreamble, d->bitstuff, samplenum, d->chanid);

	while (i < count) {
		switch (d->state) {
		case ST_DATA:
			if(d->debug)
				//printf("ST_DATA current bit is: %d  destuffed: %u total = %u \n", in[i], d->bufferpos, d->ndata);
			if (d->bitstuff) {
				if (in[i] == 1) {
					d->state = ST_STOPSIGN;// 5
					d->ndata = 0;
					d->bitstuff = 0;
				} else {
					d->ndata++;
        ...
        case ST_SKURR:	// the state when no reasonable input is coming in
        	if(d->debug)
        		//printf("ST_KURR current bit is: %d \n", in[i]);
        ...
        case ST_PREAMBLE:// when PREAMBLE has been detected
        //	puts("ST_PREAMBLE");
        	if(d->debug){
        	}//printf("ST_PREAMBLE current bit is: %d  preamble count: %u \n", in[i], d->npreamble);
		...
		case ST_STARTSIGN:
			if(d->debug)
				//printf("ST_STARTSIGN current bit is: %d \n", in[i]);
			if (d->nstartsign >= 7) {
				if (in[i] == 0) {
					d->state = ST_DATA;	// 4
                    d->startsample = samplenum;// not in original gnu ais
					d->nstartsign = 0;
					d->antallenner = 0;
		...
		case ST_STOPSIGN:
			if(d->debug)
				//printf("ST_STOPSIGN current bit is: %d \n", in[i]);
