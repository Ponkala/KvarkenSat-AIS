...
__attribute__ ((gnu_inline)) extern inline float32_t __mac_g(const float32_t *a, const float32_t *b,
			    uint32_t size)
{
	float32_t sum = 0;
	unsigned int i;
...
__attribute__ ((gnu_inline)) extern inline float32_t __mac_c(const float32_t *a, const float32_t *b,
			    unsigned int size)
{
	float f;
